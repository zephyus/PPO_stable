...
class MultiAgentOnPolicyBuffer:
    ...
    def sample_transition(self, R_bootstrap_agents, dt):
        if not self.obs:  # Empty buffer
            dummy_N = R_bootstrap_agents.shape[0] if R_bootstrap_agents is not None else 1
        ...
        if dones_arr_scalar_per_step.ndim == 1 and rewards_arr.ndim == 2:
            dones_arr = np.repeat(dones_arr_scalar_per_step[:, np.newaxis], rewards_arr.shape[1], axis=1)
        else:
        ...
        next_values = R_bootstrap_agents

        for t in reversed(range(T)):
            if t == T - 1:
                next_non_terminal = 1.0 - dones_arr[t]
                next_vals_t = next_values
            else:
                next_non_terminal = 1.0 - dones_arr[t + 1]
                next_vals_t = values_arr[t + 1]

            delta = rewards_arr[t] + self.gamma * next_vals_t * next_non_terminal - values_arr[t]
